...
const char * const BITCOIN_CONF_FILENAME = "abcoin.conf";
const char * const BITCOIN_PID_FILENAME = "abcoind.pid";
    ...
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            if (mapMultiArgs.count("-debug")) {
                ...
                // "abcoin" is a composite category enabling all Abcoin-related debug output
                if(ptrCategory->count(std::string("abcoin"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\AbcoinCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\AbcoinCore
    // Mac: ~/Library/Application Support/AbcoinCore
    // Unix: ~/.abcoincore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "AbcoinCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/AbcoinCore";
#else
    // Unix
    return pathRet / ".abcoincore";
#endif
#endif
}
...
void ReadConfigFile(const std::string& confPath)
{
    ...
    if (!streamConfig.good()){
        // Create empty abcoin.conf if it does not excist
...

    {
        ...
        {
            // Don't overwrite existing settings so command line settings override abcoin.conf
